...
\]
 То есть, если $\left(f ^ {-1}\right)'\left(a\right)  \neq 0$, то оценка $\widehat{\theta}_n\left(X_1, \cdots, X_n\right)$ асимптотически нормальна с коэффициентом $\sigma\left(\theta\right) = \left(\left(f ^ {-1}\right)'\left(a\right)\right) ^ 2 \sigma ^ 2$
 \end{enumerate}
 \begin{example}
...
\[
    \sqrt{n} \frac{\partial L}{\partial \theta} \xrightarrow{P} N\left(0, I\left(\theta_1\right)\right)
\]
Теперь разберемся с 
\[
