...
\lhead{Иванов Семен X Сафиуллин Зуфар} 
\rhead{БПМИ-183} 
...
$$ \int \frac{\varphi_\xi\left(t\right)}{\varphi_\xi\left(t\right)} = \ln \abs{\varphi_\xi\left(t\right)} + C = \int -t dt = -\frac{t ^ 2}{2}$$
...
$
    \varphi^{(k)}\left(t\right) = i ^ k  \mathbb{E}  \xi ^ k
$
\end{advice}
\begin{proof}
Заметим, что:
...
\]
Теперь очевидна непрерывность первой производной и $\varphi'\left(t\right) = i  \mathbb{E}  \xi$, для производных высших порядков аналогично
